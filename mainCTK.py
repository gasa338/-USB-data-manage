...
def copy_from_usb_to_folder(drive, destination_folder, progress_var, status_label, log_text, size_label):
    try:
        ...
        size_label.configure(text=f"Drive Size: {drive_size}")
...
def start_copy():
    ...
    if destination_folder:
        ...
        log_text = tk.Text(master=root, height=5, width=50, wrap='word')
        log_text.grid(row=20, column=0, columnspan=4, padx=5, pady=5, sticky='nsew')

        # Create Scrollbar widget
        scrollbar = ctk.CTkScrollbar(master=log_text)
        ...
        for drive in selected_drives:
            ...
            size_label = ctk.CTkLabel(root, text="Drive Size: Calculating...")
...
destination_entry = ctk.CTkEntry(root, textvariable=destination_var, width=50)
